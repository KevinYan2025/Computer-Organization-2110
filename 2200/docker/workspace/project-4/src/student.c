...
extern double priority_with_age(unsigned int current_time, pcb_t *process) {
    /* FIX ME */
    return (double)(process->priority + (current_time - process->enqueue_time ) * age_weight);
}
...
void enqueue(queue_t *queue, pcb_t *process) {
    ...
    process->enqueue_time = get_current_time(); // Set the enqueue time for age calculation

    // Calculate the "aged" priority for the process
    double process_priority = priority_with_age(get_current_time(), process);

    if (is_empty(queue)) {
        // If the queue is empty, simply place this process as the head and tail
        queue->head = process;
        queue->tail = process;
        process->next = NULL;
    } else {
        ...
        // Iterate to find the insert position where process priority is higher than the next's
        while (curr != NULL && priority_with_age(get_current_time(), curr) >= process_priority) {
            prev = curr;
            curr = curr->next;
        }

        // Insert the process into the correct position in the linked list
        if (prev == NULL) { // Insert at the head
            process->next = queue->head;
            queue->head = process;
        } else { // Insert after 'prev' and before 'curr'
            prev->next = process;
            process->next = curr;
            if (curr == NULL) { // If the current is NULL, process is the new tail
                queue->tail = process;
            }
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2 || argc > 4)
    {
      ...
      timeslice = strtoul(argv[3], NULL, 10);
